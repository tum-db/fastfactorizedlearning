...
sql factorizeSQL(const ExtendedVariableOrder* varOrder, std::ofstream& createTablesFile) {
  ...
  if (varOrder->isLeaf()) {
    ...
    sql schema{""};
    // leaf node has no key and stores schema information instead
    ...
    sql join{""}, lineage{"CONCAT('(', "}, deg{""},
        agg{"SUM(POWER(Q" +
            static_cast<const ExtendedVariableOrderNode*>(varOrder)->getChildren().front()->getName() + "." +
            name + "," + name + "d)"};
    ...
    for (const ExtendedVariableOrder* x :
         static_cast<const ExtendedVariableOrderNode*>(varOrder)->getChildren()) {
